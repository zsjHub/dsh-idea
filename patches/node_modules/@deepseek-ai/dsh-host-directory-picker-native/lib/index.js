...
/**
 * 魔改：把对话框线程的顶层窗口强行拉到前台。后台子进程弹出的
 * IFileOpenDialog 常因无前台激活权而落在窗口后面，这里用
 * AttachThreadInput + SetForegroundWindow + BringWindowToTop 组合，
 * 并带重试轮询（对话框窗口在 showing 通知之后才真正创建）。
 * @param threadId - the dialog thread's native id (from the `showing` notice).
 */
async function raiseThreadWindows(threadId) {
	const koffi = (await import("koffi")).default;
	const user32 = koffi.load("user32.dll");
	...
	const showWindow = user32.func("__stdcall", "ShowWindow", "int", ["void *", "int32"]);
	const attachThreadInput = user32.func("__stdcall", "AttachThreadInput", "int", ["uint32", "uint32", "int32"]);
	...
	const protoEnumProc = koffi.proto("int __stdcall DshRaiseEnumWndProc(void *hwnd, intptr lparam)");
	const found = [];
	const callback = koffi.register((hwnd) => {
		found.push(hwnd);
		return 1;
	...
	try {
		for (let attempt = 0; attempt < 10; attempt++) {
			...
			if (found.length > 0) {
				...
				const fgThread = fg !== null ? getWindowThreadProcessId(fg, null) : 0;
				const myThread = getCurrentThreadId();
				if (fgThread !== 0 && fgThread !== myThread) attachThreadInput(fgThread, myThread, 1);
				setForegroundWindow(hwnd);
				bringWindowToTop(hwnd);
				showWindow(hwnd, 5);
				if (fgThread !== 0 && fgThread !== myThread) attachThreadInput(fgThread, myThread, 0);
				return;
			}
		...
		worker.on("message", (message) => {
			switch (message.kind) {
				case "showing":
					...
					/* 魔改：对话框弹到最前，避免落在窗口后面要手动点。 */
					raiseThreadWindows(dialogThreadId).catch(() => void 0);
